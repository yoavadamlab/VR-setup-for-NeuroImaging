...
% read neurotar data from new iteration
%[vr.wheel_spin, vr.arduino_lick_counter, vr.trigger] = parse_arduino_data(vr);

% calculation of mouse movement 
[vr.last_wheel_spin, vr.mouse_movement, vr.speed, vr.cumsum_lap_length, vr.last_time_point, vr.speed_array] = movement_calc(vr);
...
vr.reward_time, vr.got_reward, vr.speed_array_for_stage_b] = stage_b_speed_control(vr);
...
% movement calculation 
function [last_wheel_spin, movement, speed, cumsum_lap_length, last_time_point, speed_array] = movement_calc(vr)
speed_array = vr.speed_array;
delay_me = tic;
while toc(delay_me) <0.02 % delay for 20 ms to achive 30 ms frame resolution
end
cur_wheel_spin = vr.arduino_port.UserData.mov;
mov_dif = cur_wheel_spin - vr.last_wheel_spin;
if (cur_wheel_spin == 0) && (vr.last_wheel_spin > 150)  
...
if (cur_wheel_spin < 100) && (cur_wheel_spin > 0) && (vr.last_wheel_spin > 300)
mov_dif  = (vr.p_revolution - vr.last_wheel_spin) + (cur_wheel_spin);
end
movement = vr.r * mov_dif * (2*pi/vr.p_revolution); %*-1 added by rotem
cumsum_lap_length = vr.cumsum_lap_length + movement;
last_wheel_spin = cur_wheel_spin;
if vr.timeElapsed == vr.last_time_point
speed = 0;
else
point_speed = movement / (vr.timeElapsed - vr.last_time_point);
% interpolate the last speed with two previous speeds
speed_array(1:end-1) = speed_array(2:end);
speed_array(end)= point_speed;
speed = mean(speed_array);
end
last_time_point = vr.timeElapsed;
end
...
if vr.iterations == vr.lap_end_iter + 5 || vr.stage == "A" || vr.stage == "B" % true when finish rest or just finish lap in running time
	if (vr.round_counter)  >= vr.training_round_number
...
cond = get_reward_condition(vr.stage, vr.position(2), vr.start_reward_zone, vr.resting_flag, vr.got_reward, vr.speed, vr.speed_limit, vr.reward_dealy, vr.reward_time, vr.timeElapsed, vr.currentWorld ,vr.black_screen, vr.speed_array_for_stage_b);
if cond
	% give reward
...
function cond = get_reward_condition(stage, position, start_reward_zone, resting_flag, got_reward, speed, speed_limit, reward_dealy, reward_time, timeElapsed, currentWorld ,black_screen, speed_array_for_stage_b)
	cond = false;
    switch(stage)
		case "A"
			cond = false; %never get reward at stage A
		case "B" 
			cond1 = mean(speed_array_for_stage_b) >= speed_limit; 
...
function [speed_limit, last_speed_limit_change, rewards_from_last_change, speed_array] = speed_limit_update(vr)
	speed_array = vr.speed_array_for_stage_b;
	speed_array(1:end-1) = speed_array(2:end);
	speed_array(end)= vr.speed;
	avg_speed = mean(speed_array);
	if mod(vr.iterations,300) == 0
        display("mouse average speed is: " + avg_speed + " [mm/s]");
        display("while the speed limit for reward is: " + vr.speed_limit + " [mm/s]");
    end
...
function [speed_limit, last_speed_limit_change, rewards_from_last_change, reward_time, got_reward, speed_array_for_stage_b] = stage_b_speed_control(vr)
[speed_limit, last_speed_limit_change, rewards_from_last_change, reward_time ...
,got_reward, speed_array_for_stage_b] = deal(vr.speed_limit, vr.last_speed_limit_change, ...
vr.rewards_from_last_change, vr.reward_time, vr.got_reward, vr.speed_array_for_stage_b);
if vr.stage == "B"
...
if vr.imaging_session && ~ vr.start_synchronization_clock && ~vr.first_iteration
    ...
    'O.k', 'exit', 'O.k');
    % Handle response
    saved_imaging_session = false; % prevent dubble saving
    switch answer
        case 'exit'
            saved_imaging_session = true; % prevent dubble saving
            experimentEnded = true;
    end
...
if (vr.imaging_session && toc(vr.time_from_trigger) >= vr.imaging_time) 
	...
	currentWorld = vr.black_screen;
	save_imagging_session(vr);
    saved_imaging_session = true;
    start_round = true; % for the next omagging session. thus, the resing contol mechanisem will work
end
end  
end 