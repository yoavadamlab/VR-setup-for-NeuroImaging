# 1 "C:\\Users\\Adam Lab\\Desktop\\wheel_control_WITH_VIRMEN_TO_ts_OUTPUT\\wheel_control\\wheel_control.ino"
// pin numbers
...
void setup() {
  pinMode(trigger_pin, 0x0);
  pinMode(reward_pin, 0x1);
  pinMode(lick_pin, 0x0);
  pinMode(reward_ttl_pin, 0x1);
  pinMode(lick_ttl_pin, 0x1);
  // interupt pins
  pinMode(encoder_A, 0x2); // internal pullup input pin 2
  pinMode(encoder_B, 0x2); // internal pullup input pin 3
  attachInterrupt(((encoder_A) == 2 ? 0 : ((encoder_A) == 3 ? 1 : -1)), a_rise, 3); //A rising pulse from encodenren activated ai0()
  attachInterrupt(((encoder_B) == 2 ? 0 : ((encoder_B) == 3 ? 1 : -1)), b_rise, 3); //B rising pulse from encodenren activated ai1()
  Serial.begin(38400);
...
void reward_control() {
   if (virmen_signal == REWARD_SIGNAL)
      {
      digitalWrite(reward_pin, 0x1);
      digitalWrite(reward_ttl_pin, 0x1);
      unsigned long start = millis();
      while ((millis() - start) < reward_duration) {
        }
      digitalWrite(reward_pin, 0x0);
      digitalWrite(reward_ttl_pin, 0x0);
        }
}
